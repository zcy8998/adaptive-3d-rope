...
class Attention(nn.Module):
    ...
    def forward(self, x, attn_mask=None, save_attn=False):
        ...
        if self.capture_attention or save_attn:
...
class AxialRoPEAttention3D(RoPEAttention3D):
    """Parameter-matched axial attention with shared Q/K/V projections."""
    ...
    def forward(
        ...
        if axial_coords is None:
            raise ValueError("Axial attention requires token coordinates")
        batch, tokens, channels = x.shape
        q = self.q(x).reshape(batch, tokens, self.num_heads, channels // self.num_heads)
        k = self.k(x).reshape(batch, tokens, self.num_heads, channels // self.num_heads)
        v = self.v(x).reshape(batch, tokens, self.num_heads, channels // self.num_heads)
        if freqs_cis is not None:
            q, k = apply_rotary_emb(q, k, freqs_cis)
        q = q.permute(0, 2, 1, 3)
        k = k.permute(0, 2, 1, 3)
        ...
        for axis in range(3):
            ...
            same_line = (
                axial_coords[:, :, None, other_axes]
                == axial_coords[:, None, :, other_axes]
            ).all(dim=-1)
            allowed = same_line
            if attn_mask is not None:
                allowed = allowed & attn_mask[:, None, :]
            axis_logits = logits.masked_fill(
                ~allowed.unsqueeze(1), torch.finfo(logits.dtype).min
            )
            attention = self.attn_drop(axis_logits.softmax(dim=-1))
            if self.capture_attention or save_attn:
                captured.append(attention.detach().cpu())
            outputs.append(attention @ v)

        if captured:
            self.last_attn_map = torch.stack(captured, dim=0)
