...
def train_one_epoch_csi(model: torch.nn.Module,
                    ...
                    log_writer=None,
                    args=None):
    # A positional probe optimizes only explicitly installed scale parameters.
    # Keeping the otherwise frozen model in eval mode prevents dropout or
    # drop-path noise from being mistaken for a positional effect.  Autograd is
    # intentionally left enabled, so the probe parameters still receive
    # gradients.
    model.train(not bool(getattr(args, "frozen_probe_eval_mode", False)))
    metric_logger = misc.MetricLogger(delimiter=" ")
    metric_logger.add_meter('lr', misc.SmoothedValue(window_size=1, fmt='{value:.6f}'))
    header = 'Epoch: [{}]'.format(epoch)
    print_freq = 100
